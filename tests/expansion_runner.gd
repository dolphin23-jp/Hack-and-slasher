...
func clear_combat()->void:
 for list in [game.enemies,game.projectiles,game.drops]:
  ...
  list.clear()
 game.player.skill_actions.clear();game.player.finisher_charge=0;game.player.reset_chain()
...
func run()->void:
 ...
 clear_combat();game.player.level=2;game.pending_upgrades=1;game.player.upgrades={};game.prepare_upgrade()
 var native_keys=game.upgrade_choices.map(func(c):return String(c.key))
 expect("level up includes a current-chain native blessing",native_keys.any(func(k):return k.begins_with("master_") or k in ["transition_power","slot2_reach","triune_mastery","same_family_mastery","harvest_cast_mastery"]))
 var choices=BuildDB.available({},[])
 expect("new runs offer chain skills instead of irreversible lance branches",not choices.any(func(c):return c.get("family","")=="lance") and choices.any(func(c):return c.key=="art_power"))
 clear_combat();game.player.equipment.weapon.weapon_type="spear";game.player.cast(0)
 expect("spear art launches a wide piercing projectile",game.projectiles.size()==1 and game.projectiles[0].radius==18 and game.projectiles[0].pierce==12)
 clear_combat();game.player.equipment.weapon.weapon_type="staff";e=enemy("hollow",Vector2(90,0));var neighbor=enemy("hollow",Vector2(160,0));var hp_before=neighbor.hp
 game.player.cast(0);game.projectiles[0].tick(.15)
 expect("staff cannon pierces several enemies",e.hp<10000 and neighbor.hp<hp_before)
 clear_combat();equip_legend(4);game.player.equipment.weapon.weapon_type="spear";game.player.cast(0)
 expect("legacy side-shot artifact applies to spear art",game.projectiles.size()==3)
 clear_combat();equip_legend(13);game.player.equipment.weapon.weapon_type="spear";game.player.cast(0);var bolt=game.projectiles[0];var original_damage=bolt.damage;bolt.life=.01;bolt.tick(.02)
 expect("return art reverses once with reduced damage",bolt.returning and bolt.velocity.x<0 and is_equal_approx(bolt.damage,original_damage*.6))
 bolt.life=.01;bolt.tick(.02)
 expect("return art expires instead of looping",bolt.dead)
 ...
 clear_combat();game.player.active_oaths=["storm"];equip_legend(0);equip_legend(7);e=enemy();neighbor=enemy("hollow",Vector2(170,30));hp=neighbor.hp;game.player.equipment.weapon.weapon_type="spear";game.player.cast(0);game.projectiles[0].tick(.07)
 expect("storm oath adds lightning to spear art impact",neighbor.hp<hp)
