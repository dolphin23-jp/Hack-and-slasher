...
func run()->void:
 ...
 game.player.upgrades={"lance_fan":1.0}
 var remaining_lance=BuildDB.available(game.player.upgrades,game.profile.chronicle.achievements).filter(func(c):return c.get("family","")=="lance")
 expect("one irreversible lance branch remains enforced",remaining_lance.is_empty())
 game.player.cast(2)
 expect("legacy fan path still changes projectile count",game.projectiles.size()==3)
