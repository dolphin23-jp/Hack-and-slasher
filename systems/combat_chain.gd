...
static func strike(p)->void:
 ...
 if p.combo==3:
  ...
  if p.has_unique("shield_spend_power") and p.barrier>=10:
   p.barrier-=10;amount*=1.35 if p.unique_rarity("shield_spend_power")==4 else 1.25
 ...
 if p.has_unique("full_shield_magic_double") and "magic" in w.types and p.stats.shield_max>0 and p.barrier>=p.stats.shield_max*.9:rounds+=2 if p.unique_rarity("full_shield_magic_double")==4 else 1
 var landed=0
  ...
  if w.shape in ["bolt","wave"]:
   ...
   for hit in range(w.hits):
    for e in g.enemies.duplicate():
     ...
     var unique_crit=.12 if p.unique_rarity("transition_crit")==4 else .08
     var crit_chance=float(p.stats.crit)+(unique_crit if not String(transition.id).is_empty() and p.has_unique("transition_crit") else 0.0)
